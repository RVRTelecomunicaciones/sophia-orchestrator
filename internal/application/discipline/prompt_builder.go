...
// Note: PromptInput.Skills was removed in M3 PR3a (D-M3-5). Skills are now
// rendered inside PriorContext via PriorContext.Skills → Render. The caller
// (phase/service.go, apply/teamlead.go) attaches RenderedSkill items to the
// PriorContext struct before calling Render and passing the result as
// PromptInput.PriorContext.
...
func (pb *PromptBuilder) Build(in PromptInput) (string, error) {
	...
	// M3 D-M3-5: # Skill section removed from prompt_builder.
	// Skills are now rendered inside PriorContext via PriorContext.Skills → Render.
	// The caller attaches skills to the PriorContext struct before passing
	// the rendered string as PromptInput.PriorContext.
