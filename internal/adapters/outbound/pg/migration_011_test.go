...
func TestMigration011_PreState(t *testing.T) {
	...
	// Apply exactly up to migration 010 so we get the pre-011 baseline.
...
func TestMigration011_PostUp(t *testing.T) {
	pool := setupSkillPG(t) // applies all migrations including 011
	ctx := context.Background()

	// Verify skill_usage table exists (created by migration 011).
	var tableExists bool
	err := pool.QueryRow(ctx, `
		SELECT EXISTS(
			SELECT 1
