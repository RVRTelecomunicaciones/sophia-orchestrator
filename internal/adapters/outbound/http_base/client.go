...
func (c *Client) do(ctx context.Context, method, path string, body, out any) error {
	...
	result, err := c.cb.Execute(func() (*Result, error) {
		res, retryErr := c.executeWithRetry(ctx, method, fullURL, payload)
		if retryErr != nil {
			return nil, retryErr
		}
...
func (c *Client) executeWithRetry(ctx context.Context, method, fullURL string, payload []byte) (*Result, error) {
	...
	for attempt := 0; attempt < c.cfg.MaxAttempts; attempt++ {
		req, err := buildRequest(ctx, method, fullURL, payload, c.cfg.DefaultHeaders)
		if err != nil {
			return nil, err
		}
...
func (c *Client) computeBackoff(attempt int) time.Duration {
	...
	if c.cfg.JitterFraction > 0 {
		...
		offset := (rand.Float64()*2 - 1) * jitter //nolint:gosec // backoff jitter, not crypto
...
func buildRequest(ctx context.Context, method, fullURL string, payload []byte, defaultHeaders http.Header) (*http.Request, error) {
