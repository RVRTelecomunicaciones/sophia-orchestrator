...
Status mutation MUST require an explicit apply/confirm invocation distinct from dry-run (separate flag or separate command). When applied, only the skills identified by the recomputed gates MUST have their status changed (promote or demote); all other skills MUST remain untouched. Operators MUST have a documented, validated path to reverse a confirmed mass shift.

> Spec reconciliation (loop-hardening archive, 2026-06-16): the original requirement said "the same command MUST be able to reverse a confirmed change (rollback path)". The shipped implementation does NOT add a reverse surface to the `reeval` command; reversal is delegated to the existing admin `PATCH /api/v1/skills/{id}/status` transition (the validated 6-enum `allowedTransitions` chain), documented in the CLI help text and report footer. This is a real, operator-usable recovery path but it is multi-hop, not single-invocation. The literal "same command" MUST is NOT satisfied. This gap is recorded as tracked FOLLOW-UP-1 (see the loop-hardening archive report): a future change MUST either add a `reeval --revert` single-invocation undo (via a prior-status snapshot) OR formally amend this MUST to a SHOULD accepting the PATCH-chain delegation.
