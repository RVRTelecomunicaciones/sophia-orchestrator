...
## Scope boundary (as shipped — MVP)

- Critic runs only on **single-agent enveloped phases** (`runAsync`, between
  `Validator.Validate` and `p.Complete`). The apply phase (`runApplyPhase`,
  which emits the slimmer `PhaseCompletedFromApplyPayload`) is OUT of scope — a
  deferred follow-up.
- Concern persistence is **SSE-only**: `done_with_concerns` persists on the
  phase row, but the `Concern` detail rides the SSE payload and is not durably
  stored. Durable concern persistence is a deferred follow-up.
- The MVP critic is the **deterministic `StubCritic`** (envelope-derived rules:
  `Risk{Level:"high"}`→`Concern{high,risk}`, `Confidence<0.5`→`Concern{medium,confidence}`;
  no clock/random). A real LLM critic via the OpenCode dispatcher is deferred.
